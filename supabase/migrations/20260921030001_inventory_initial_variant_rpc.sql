...
begin
  ...
  if p_initial_stock > 0 then
    select r.receipt_id into v_receipt from public.receive_inventory_stock(p_variant,p_initial_stock,p_initial_cost,null,null,now(),'Initial stock',p_actor_id,p_idempotency_key,'INITIAL') r;
  end if;
  return query select v_variant,v_receipt;
end;
$$;
revoke execute on function public.create_admin_variant_with_initial_stock(uuid,text,text,text,text,text,numeric,numeric,integer,boolean,integer,numeric,uuid,uuid) from public,anon,authenticated;
grant execute on function public.create_admin_variant_with_initial_stock(uuid,text,text,text,text,text,numeric,numeric,integer,boolean,integer,numeric,uuid,uuid) to service_role;