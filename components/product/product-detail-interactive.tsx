  ...
  return (
    <div className="grid gap-10 lg:grid-cols-[minmax(0,.92fr)_minmax(0,1.08fr)] lg:gap-16">
      <div className="min-w-0">
        <div className="relative flex aspect-square items-center justify-center overflow-hidden rounded-[2rem] border border-slate-200 bg-[radial-gradient(circle_at_50%_35%,#ffffff_0%,#f6f7f9_58%,#e9edf3_100%)] p-6 shadow-sm sm:p-10">
          {imageUrl ? (
            ...
            <div className="text-center" role="img" aria-label={`${product.name} image unavailable`}>
              <div className="mx-auto flex h-32 w-32 items-center justify-center rounded-[2rem] bg-slate-950 text-4xl font-black tracking-[-0.08em] text-orange-300 shadow-2xl shadow-slate-950/20 sm:h-36 sm:w-36 sm:rounded-[2.5rem] sm:text-5xl">{product.name.slice(0, 2).toUpperCase()}</div>
              <p className="mt-3 text-[10px] font-bold uppercase tracking-[0.22em] text-slate-400 sm:text-[11px]">Product image coming soon</p>
            </div>
          )}
          {discount ? <span className="absolute left-5 top-5 rounded-full bg-slate-950 px-3.5 py-1.5 text-xs font-black text-orange-300 shadow-lg">-{discount}%</span> : null}
          {imageCount > 1 ? (
            <>
              <button type="button" onClick={() => selectImage(activeImageIndex - 1)} disabled={activeImageIndex === 0} aria-label="Previous product image" className="absolute left-3 top-1/2 inline-flex h-10 w-10 -translate-y-1/2 items-center justify-center rounded-full border border-white/70 bg-white/90 text-slate-900 shadow-sm transition hover:bg-white disabled:pointer-events-none disabled:opacity-0 focus-visible:outline-none focus-visible:ring-4 focus-visible:ring-orange-200"><ChevronLeft className="h-5 w-5" /></button>
              <button type="button" onClick={() => selectImage(activeImageIndex + 1)} disabled={activeImageIndex === imageCount - 1} aria-label="Next product image" className="absolute right-3 top-1/2 inline-flex h-10 w-10 -translate-y-1/2 items-center justify-center rounded-full border border-white/70 bg-white/90 text-slate-900 shadow-sm transition hover:bg-white disabled:pointer-events-none disabled:opacity-0 focus-visible:outline-none focus-visible:ring-4 focus-visible:ring-orange-200"><ChevronRight className="h-5 w-5" /></button>
            </>
          ) : null}
        </div>

        {imageCount > 1 ? <div className="mt-4 flex gap-3 overflow-x-auto pb-2" aria-label="Product image gallery">{displayImages.map((image, index) => <button key={image.id} type="button" onClick={() => selectImage(index)} aria-label={`View product image ${index + 1}`} aria-current={activeImageIndex === index ? 'true' : undefined} className={`relative h-20 w-20 shrink-0 overflow-hidden rounded-2xl border-2 bg-white p-1.5 transition duration-150 focus-visible:outline-none focus-visible:ring-4 focus-visible:ring-orange-200 ${activeImageIndex === index ? 'border-orange-600 ring-2 ring-orange-100' : 'border-slate-200 hover:border-slate-400'}`}>
      ...
      <div className="min-w-0">
        {product.brand ? <Link href={getBrandPath(product.brand.slug)} className="inline-flex items-center gap-2 text-[10px] font-black uppercase tracking-[0.2em] text-orange-600 transition-colors hover:text-orange-700 focus-visible:outline-none focus-visible:ring-4 focus-visible:ring-orange-200 sm:text-xs"><BrandLogo brand={product.brand} size="sm" className="h-8 w-8 rounded-lg p-1" /><span>{product.brand.name}</span><span className="normal-case tracking-normal text-slate-400">View brand</span></Link> : <p className="text-[10px] font-black uppercase tracking-[0.2em] text-orange-600 sm:text-xs">PhonerBazar</p>}
        <div className="mt-3 flex flex-wrap items-center gap-x-3 gap-y-1 text-xs font-semibold text-slate-500"><span>{getProductTypeLabel(product.product_type)}</span>{product.category ? <><span aria-hidden="true">·</span><Link href={`/products?category=${encodeURIComponent(product.category.slug)}`} className="transition-colors hover:text-orange-700 focus-visible:outline-none focus-visible:ring-4 focus-visible:ring-orange-200">{product.category.name}</Link></> : null}</div>
        ...
        <div className="mt-7 flex flex-wrap items-baseline gap-3"><p className="break-words text-3xl font-black tracking-tight text-slate-950 sm:text-4xl">{selected ? formatPrice(selected.price) : 'Price on request'}</p>{selected?.compare_at_price && selected.compare_at_price > selected.price ? <><p className="text-sm text-slate-400 line-through">{formatPrice(selected.compare_at_price)}</p><span className="rounded-full bg-orange-50 px-3 py-1 text-xs font-black text-orange-700">Save {formatPrice(selected.compare_at_price - selected.price)} · {discount}% off</span></> : null}</div>
        <div className="mt-2 flex flex-wrap items-center gap-2 text-xs"><span className={`rounded-full px-2.5 py-1 font-bold ${status.tone === 'in' ? 'bg-orange-50 text-orange-700' : status.tone === 'low' ? 'bg-amber-50 text-amber-700' : 'bg-slate-100 text-slate-500'}`}>{status.label}</span>{selected?.sku ? <span className="text-slate-400">SKU: <span className="font-semibold text-slate-600">{selected.sku}</span></span> : null}</div>

        {product.variants.length > 0 ? <div className="mt-8"><div className="flex items-center justify-between gap-3"><p className="text-sm font-black text-slate-950">Choose a variant</p><span className="text-xs font-semibold text-slate-400">{product.variants.length} option{product.variants.length === 1 ? '' : 's'}</span></div><div className="mt-3 grid gap-2 sm:grid-cols-2">{product.variants.map((variant) => { const variantStatus = getPublicAvailability([variant]); const isSelected = selected?.id === variant.id; const isDisabled = !variant.is_in_stock && !isSelected; return <button key={variant.id} type="button" onClick={() => selectVariant(variant.id)} disabled={isDisabled} aria-pressed={isSelected} className={`flex min-h-16 flex-wrap items-center justify-between gap-2 rounded-2xl border p-4 text-left transition duration-150 focus-visible:outline-none focus-visible:ring-4 focus-visible:ring-orange-200 disabled:cursor-not-allowed disabled:opacity-50 ${isSelected ? 'border-orange-500 bg-orange-50 ring-2 ring-orange-100' : 'border-slate-200 bg-white hover:border-slate-400'}`}><span className="min-w-0 flex-1"><span className="block break-words font-bold text-slate-950">{getVariantLabel(variant)}</span><span className="mt-1 block break-all text-xs text-slate-500">{formatPrice(variant.price)}{variant.sku ? ` · ${variant.sku}` : ''}</span></span><span className={`shrink-0 text-[10px] font-black uppercase tracking-[0.12em] ${variantStatus.tone === 'in' ? 'text-orange-700' : variantStatus.tone === 'low' ? 'text-amber-700' : 'text-slate-400'}`}>{variantStatus.label}</span></button> })}</div></div> : <p className="mt-8 rounded-2xl border border-amber-200 bg-amber-50 p-4 text-sm font-semibold text-amber-800">This product does not currently have a purchasable variant.</p>}
      ...
      <div className="fixed inset-x-0 bottom-0 z-40 border-t border-slate-200 bg-white/95 px-3 py-2 shadow-[0_-8px_24px_rgba(15,23,42,0.08)] backdrop-blur md:hidden [padding-bottom:max(0.5rem,env(safe-area-inset-bottom))]">
        <div className="mx-auto flex max-w-xl items-center gap-2">
          ...
          <button type="button" disabled={!selected || !selected.is_in_stock || cartBusy || buyBusy} onClick={buySelectedNow} className="min-h-11 rounded-xl bg-[#FF6B00] px-4 text-xs font-black text-white disabled:opacity-50">{buyBusy ? 'Opening…' : 'Buy Now'}</button>
        </div>
      </div>
    </div>
  )
