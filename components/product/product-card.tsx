...
export function ProductMedia({ product, className = '', priority = false }: { product: StorefrontProduct; className?: string; priority?: boolean }) {
  ...
  return <div className={`relative flex aspect-square items-center justify-center overflow-hidden rounded-lg bg-[#f6f7f9] sm:rounded-2xl ${className}`}>
...
export function ProductCard({ product, priority = false }: { product: StorefrontProduct; priority?: boolean }) {
  ...
  return <article className="group flex min-w-0 h-full flex-col overflow-hidden rounded-xl border border-slate-200 bg-white p-1.5 shadow-[0_4px_18px_rgba(15,23,42,0.045)] transition-all duration-200 hover:-translate-y-0.5 hover:border-orange-200 hover:shadow-[0_10px_30px_rgba(15,23,42,0.08)] sm:rounded-2xl sm:p-3">
