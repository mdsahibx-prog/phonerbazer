...
export function ProductMedia({ product, className = '', priority = false }: { product: StorefrontProduct; className?: string; priority?: boolean }) {
  ...
  return <div className={`relative flex aspect-square items-center justify-center overflow-hidden rounded-lg bg-[#f7f7f5] sm:rounded-2xl ${className}`}>
...
export function ProductCard({ product, priority = false }: { product: StorefrontProduct; priority?: boolean }) {
  ...
  return <article className="group flex min-w-0 h-full flex-col overflow-hidden rounded-xl border border-black/[0.08] bg-white p-1.5 shadow-[0_4px_18px_rgba(0,0,0,0.06)] transition-all duration-200 hover:-translate-y-0.5 hover:border-orange-300 hover:shadow-[0_10px_30px_rgba(0,0,0,0.10)] sm:rounded-2xl sm:p-3">
    ...
    <div className="flex min-w-0 flex-1 flex-col px-0.5 pb-0.5 pt-1.5 sm:px-2 sm:pb-2 sm:pt-3">
      ...
      <Link href={`/products/${product.slug}`} className="mt-1 block focus-visible:outline-none focus-visible:ring-4 focus-visible:ring-orange-100 sm:mt-2"><h2 className="line-clamp-2 min-h-[2.1rem] text-[11px] font-black leading-[1.05rem] tracking-tight text-black sm:min-h-0 sm:text-base sm:leading-6">{product.name}</h2></Link>
      <div className="mt-auto pt-1.5 sm:pt-5"><div className="flex min-w-0 flex-wrap items-baseline gap-x-1 gap-y-0.5 sm:gap-2"><span className="min-w-0 truncate text-[12px] font-black leading-4 tracking-tight text-black sm:text-base sm:leading-normal">{priceRange ? `From ${priceRange}` : 'Price on request'}</span>{compareAt ? <span className="text-[9px] font-medium leading-4 text-slate-400 line-through sm:text-xs">{formatPrice(compareAt)}</span> : null}</div><ProductPurchaseActions product={product} /></div>
    </div>
  </article>
}
