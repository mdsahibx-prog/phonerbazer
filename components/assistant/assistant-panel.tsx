...
function getSessionId() {
  const key = 'phonerbazar-assistant-session'
...
export default function AssistantPanel({ onClose, assistantName = 'PhonerBazar Assistant', subtitle = 'বাংলায় পণ্য ও স্টোর সহায়তা', maintenanceMode = false, maintenanceMessage, maxVisibleProductCards = 4, showQuickPrompts = true, welcomeMessage, quickPrompts }: { onClose: () => void; assistantName?: string; subtitle?: string; maintenanceMode?: boolean; maintenanceMessage?: string; maxVisibleProductCards?: number; showQuickPrompts?: boolean; welcomeMessage?: string; quickPrompts?: string[] }) {
  const titleId = useId()
  ...
  return (
    ...
    <div id="phonerbazar-assistant-panel" ref={panelRef} role="dialog" aria-modal="true" tabIndex={-1} aria-labelledby={titleId} aria-describedby={descriptionId} className="fixed inset-x-3 bottom-[calc(5.5rem+env(safe-area-inset-bottom))] z-50 flex max-h-[min(70vh,620px)] flex-col overflow-hidden rounded-3xl border border-slate-200 bg-white shadow-[0_24px_80px_rgba(15,23,42,0.24)] sm:inset-x-auto sm:right-4 sm:w-[min(390px,calc(100vw-2rem))] motion-safe:animate-in motion-safe:fade-in motion-safe:slide-in-from-bottom-2 motion-safe:duration-200 motion-reduce:animate-none">
      ...
      <div className="flex-1 space-y-3 overflow-y-auto p-4" aria-live="polite" aria-busy={loading}>{maintenanceMode ? <div role="status" className="rounded-2xl border border-amber-200 bg-amber-50 px-3 py-3 text-sm leading-6 text-amber-900">{maintenanceMessage || 'সহকারীটি বর্তমানে সাময়িকভাবে বন্ধ আছে।'}</div> : null}
        {messages.length === 0 ? <div className="space-y-4"><p className="text-sm leading-6 text-slate-700">{welcomeMessage || 'হ্যালো। আমি PhonerBazar-এর প্রকাশ্য পণ্য, মূল্য, প্রাপ্যতা, ডেলিভারি ও ওয়ারেন্টি তথ্য খুঁজে দিতে পারি।'}</p>{showQuickPrompts ? <div className="grid gap-2">{(quickPrompts?.length ? quickPrompts : defaultPrompts).map((prompt) => <button type="button" key={prompt} onClick={() => submit(prompt)} className="rounded-2xl border border-slate-200 px-3 py-2.5 text-left text-sm text-slate-700 transition hover:border-teal-400 hover:bg-teal-50 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-teal-600">{prompt}</button>)}</div> : null}</div> : null}
