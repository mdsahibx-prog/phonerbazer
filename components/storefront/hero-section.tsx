  ...
  if (banners.length === 0) {
    return (
      <section className="relative overflow-hidden bg-slate-950 text-white">
        <div className="absolute inset-0 bg-[radial-gradient(circle_at_80%_15%,rgba(249,115,22,0.24),transparent_35%),radial-gradient(circle_at_10%_90%,rgba(255,255,255,0.08),transparent_35%)]" />
        <div className="relative mx-auto grid max-w-7xl items-center gap-10 px-4 py-12 sm:px-6 sm:py-20 lg:grid-cols-[1.1fr_0.9fr] lg:px-8 lg:py-24">
          <div><div className="inline-flex items-center gap-2 rounded-full border border-orange-400/30 bg-orange-400/10 px-4 py-2 text-[10px] font-bold uppercase tracking-[0.18em] text-orange-300 sm:text-xs"><Zap className="h-3.5 w-3.5" /> Trusted mobile & gadget store in Bangladesh</div><h1 className="mt-6 max-w-3xl text-3xl font-black leading-[1.05] tracking-[-0.05em] sm:text-5xl md:text-6xl lg:text-7xl">Authentic devices.<br /><span className="text-emerald-300">Clear pricing. No guesswork.</span></h1><p className="mt-6 max-w-xl text-sm leading-7 text-slate-300 sm:text-base sm:leading-8">{siteConfig.tagline}. {siteConfig.brandPromise}. Explore verified mobile phones, feature phones, and smart wearables with secure Cash on Delivery nationwide.</p><div className="mt-8 flex flex-wrap gap-3"><Link href="/products" className="inline-flex min-h-12 items-center gap-2 rounded-full bg-orange-500 px-7 text-sm font-black text-slate-950 hover:bg-orange-400">Explore catalogue <ArrowRight className="h-4 w-4" /></Link><Link href="/categories" className="inline-flex min-h-12 items-center rounded-full border border-white/15 px-7 text-sm font-bold text-white hover:bg-white/5">Browse categories</Link></div></div>
...
function StatsCard({ productCount, brandCount, categoryCount }: { productCount: number; brandCount: number; categoryCount: number }) {
  return <div className="relative mx-auto w-full max-w-md lg:ml-auto"><div className="absolute -inset-4 rounded-[2.5rem] bg-emerald-400/10 blur-2xl" /><div className="relative overflow-hidden rounded-[2rem] border border-white/10 bg-slate-900/60 p-6 shadow-2xl shadow-black/40 backdrop-blur-xl sm:p-7"><div className="flex items-center justify-between border-b border-white/10 pb-5"><div><p className="text-[10px] font-bold uppercase tracking-[0.2em] text-orange-300 sm:text-xs">Store snapshot</p><p className="mt-1 text-2xl font-black tracking-tight">{siteConfig.name}</p></div><div className="relative flex h-12 w-12 shrink-0 items-center justify-center overflow-hidden rounded-2xl bg-slate-950 shadow-md"><Image src="/phonerbazar-icon.svg" alt="PhonerBazar Logo" width={48} height={48} className="h-full w-full object-cover" /></div></div><div className="mt-6 grid grid-cols-2 gap-3"><div className="rounded-2xl bg-white/[0.06] p-4"><p className="text-2xl font-black text-white sm:text-3xl">{productCount}</p><p className="mt-1 text-[10px] text-slate-300 sm:text-xs">Active products</p></div><div className="rounded-2xl bg-white/[0.06] p-4"><p className="text-2xl font-black text-white sm:text-3xl">{brandCount}</p><p className="mt-1 text-[10px] text-slate-300 sm:text-xs">Verified brands</p></div><div className="rounded-2xl bg-white/[0.06] p-4"><p className="text-2xl font-black text-white sm:text-3xl">{categoryCount}</p><p className="mt-1 text-[10px] text-slate-300 sm:text-xs">Categories</p></div><div className="rounded-2xl bg-orange-500 p-4 text-slate-950"><p className="text-xs font-bold uppercase tracking-wider text-slate-800">Support</p><p className="mt-1 truncate text-xs font-black">{siteConfig.contact.phone}</p></div></div><div className="mt-6 rounded-2xl border border-orange-400/20 bg-orange-400/10 p-4"><p className="text-xs font-bold text-orange-200">{siteConfig.brandPromise}</p><p className="mt-1.5 text-[10px] leading-5 text-slate-300">Clear specs, valid warranty terms, and prompt delivery across Bangladesh.</p></div></div></div>
}
