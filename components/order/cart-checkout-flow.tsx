  ...
  return <form onSubmit={submitOrder} className="grid gap-8 lg:grid-cols-[minmax(0,1fr)_22rem]">
    <section className="rounded-[1.75rem] border border-slate-200 bg-white p-6 shadow-sm sm:p-8">
      ...
      {step === 'details' ? <>
        ...
        <div className="mt-7 rounded-2xl bg-slate-950 p-5 text-white"><div className="flex items-start gap-3"><ShieldCheck className="mt-0.5 h-5 w-5 text-orange-300" /><div><p className="font-black">Server-controlled payment routing</p><p className="mt-1 text-xs leading-5 text-slate-300">Cash on Delivery remains available when permitted. If verification requires advance payment, you will be redirected to the configured secure provider.</p></div></div></div>
        ...
        <div className="mt-6 grid gap-3 sm:grid-cols-2"><button type="button" disabled={busy} onClick={() => setStep('details')} className="inline-flex min-h-12 items-center justify-center gap-2 rounded-full border border-slate-300 px-5 py-3 text-sm font-black text-slate-700"><ArrowLeft className="h-4 w-4" /> Edit details</button><button type="submit" disabled={busy} className="inline-flex min-h-12 items-center justify-center gap-2 rounded-full bg-orange-500 px-5 py-3 text-sm font-black text-slate-950 disabled:opacity-50">{busy ? <><LoaderCircle className="h-4 w-4 animate-spin" /> Confirming…</> : <>Confirm order <CheckCircle2 className="h-4 w-4" /></>}</button></div>
      </>}
    </section>
