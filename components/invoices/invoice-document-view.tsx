...
export async function InvoiceDocumentView({ invoice }: { invoice: InvoiceDocument }) {
  ...
  return <article id="invoice-preview" className="invoice-paper mx-auto max-w-[794px] bg-white p-6 text-slate-950 shadow-xl shadow-slate-950/10 print:max-w-none print:p-0 print:shadow-none">
    ...
    {(invoice.warrantyPolicy || invoice.returnRefundPolicy) && <section className="invoice-policy mt-4 rounded-xl border border-orange-100 bg-orange-50/60 p-3"><h2 className="text-[10px] font-black uppercase tracking-[0.14em] text-emerald-700">Policy note</h2>{invoice.warrantyPolicy && <p className="mt-1 text-[10px] leading-4 text-slate-700">{invoice.warrantyPolicy}</p>}{invoice.returnRefundPolicy && <p className="mt-1 text-[10px] leading-4 text-slate-600">{invoice.returnRefundPolicy}</p>}</section>}
    <footer className="invoice-footer mt-4 border-t border-slate-200 pt-3 text-[9px] leading-4 text-slate-500">Please retain this invoice for order and warranty reference. {invoice.storeProfile.businessName} · {invoice.storeProfile.location} · {invoice.storeProfile.phone} · {invoice.storeProfile.publicEmail}</footer>
  </article>
}
