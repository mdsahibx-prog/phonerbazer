...
function initializeTikTok(pixelId: string) { const w = window as typeof window & { TiktokAnalyticsObject?: string; ttq?: any }; if (!pixelId) return; w.TiktokAnalyticsObject = 'ttq'; const ttq = w.ttq = w.ttq || []; ttq.methods = ttq.methods || ['page','track','identify','instances','debug','on','off','once','ready','alias','group','enableCookie','disableCookie']; ttq.setAndDefer = ttq.setAndDefer || function(t:any,e:string){t[e]=function(...args:any[]){t.push([e,...args])}}; ttq.methods.forEach((method:string)=>ttq.setAndDefer(ttq,method)); ttq.instance = ttq.instance || function(id:string){const instance=ttq._i?.[id]||[]; ttq._i=ttq._i||{}; ttq._i[id]=instance; ttq.methods.forEach((method:string)=>ttq.setAndDefer(instance,method)); return instance}; ttq.load = ttq.load || function(id:string){ttq._i=ttq._i||{}; ttq._i[id]=ttq._i[id]||[]; ttq._i[id]._u='https://analytics.tiktok.com/i18n/pixel/events.js'; ttq._t=ttq._t||{}; ttq._t[id]=+new Date; ttq._o=ttq._o||{}; ttq._o[id]={}; const script=document.createElement('script'); script.async=true; script.src='https://analytics.tiktok.com/i18n/pixel/events.js?sdkid='+encodeURIComponent(id)+'&lib=ttq'; document.head.appendChild(script)}; if (!initializedTikTokPixelIds.has(pixelId)) { ttq.load(pixelId); initializedTikTokPixelIds.add(pixelId); ttq.page() } }\nfunction loadScript(src: string, idValue: string) { if (document.getElementById(idValue)) return; const script = document.createElement('script'); script.id = idValue; script.async = true; script.src = src; document.head.appendChild(script) }
...
export function trackClientEvent(input: ClientEventInput) {
  ...
  if (gtmRuntimeActive && (currentConsent.analytics || input.testMode)) {
    initializeGtm()
    w.dataLayer = w.dataLayer || []
  ...
  if (runtimeConfig.enabled && runtimeConfig.marketingEnabled && (currentConsent.marketing || input.testMode) && runtimeConfig.tiktokPixelId) { initializeTikTok(runtimeConfig.tiktokPixelId); const w = window as typeof window & { ttq?: any }; const ttMap: Record<string,string> = { page_view:'PageView', view_item:'ViewContent', view_item_list:'ViewContent', search:'Search', add_to_cart:'AddToCart', begin_checkout:'InitiateCheckout', add_shipping_info:'AddPaymentInfo', add_payment_info:'AddPaymentInfo', purchase:'CompletePayment', generate_lead:'SubmitForm', contact:'Contact', sign_up:'CompleteRegistration', whatsapp_click:'Contact' }; const ttEvent = ttMap[event.eventName]; if (ttEvent && w.ttq?.track) w.ttq.track(ttEvent, { ...event.commerce, event_id: event.eventId }) }\n  if (runtimeConfig.enabled && (currentConsent.analytics || input.testMode)) { const idValue = runtimeConfig.ga4MeasurementId; if (idValue) { const w = window as typeof window & { gtag?: (...args: unknown[]) => void }; w.gtag = w.gtag || function (...args: unknown[]) { (w as typeof w & { dataLayer?: unknown[] }).dataLayer = (w as typeof w & { dataLayer?: unknown[] }).dataLayer || []; (w as typeof w & { dataLayer?: unknown[] }).dataLayer?.push(args) }; loadScript(`https://www.googletagmanager.com/gtag/js?id=${encodeURIComponent(idValue)}`, 'sahigadget-ga4'); if (initializedGa4MeasurementId !== idValue) { w.gtag('js', new Date()); w.gtag('config', idValue, { send_page_view: false }); initializedGa4MeasurementId = idValue } w.gtag('event', event.eventName, { ...event.commerce, event_id: event.eventId }) } }
