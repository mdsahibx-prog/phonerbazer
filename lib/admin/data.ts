...
export async function getProductManagementData() {
  ...
  const [products, brands, categories, images] = await Promise.all([
    db.from('products').select('id, name, slug, status, is_published, is_featured, product_type, warranty_policy, brand_id, category_id, created_at, updated_at, brands(name), categories(name), product_variants(id, sku, variant_title, color, price, compare_at_price, stock_quantity, low_stock_threshold, is_active)').order('updated_at', { ascending: false }).limit(100),
