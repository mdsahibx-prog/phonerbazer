...
import { AlertTriangle, Banknote, CheckCircle2, ClipboardList, PackageCheck, PackageX, ShieldAlert, ShoppingCart, Truck, Users, ArrowUpRight, Clock3 } from 'lucide-react'
...
export default async function AdminDashboardPage() {
  ...
  return <div>
    ...
    <section className="mt-6 grid gap-4 md:grid-cols-3">
      <div className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm"><div className="flex items-center gap-2 text-slate-500"><ArrowUpRight className="h-4 w-4" /><span className="text-xs font-bold uppercase tracking-wider">Sales momentum</span></div><p className="mt-3 text-2xl font-semibold text-slate-950">{salesChange === null ? '—' : `${salesChange >= 0 ? '+' : ''}${salesChange.toFixed(1)}%`}</p><p className="mt-1 text-xs text-slate-500">Today vs previous day revenue</p></div>
      <div className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm"><div className="flex items-center gap-2 text-slate-500"><Clock3 className="h-4 w-4" /><span className="text-xs font-bold uppercase tracking-wider">Attention queue</span></div><p className="mt-3 text-2xl font-semibold text-slate-950">{data.pendingOrders + data.lowStock.length + commerce.highRiskToday}</p><p className="mt-1 text-xs text-slate-500">Pending + low stock + high-risk signals</p></div>
      <div className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm"><div className="flex items-center gap-2 text-slate-500"><CheckCircle2 className="h-4 w-4" /><span className="text-xs font-bold uppercase tracking-wider">Conversion signal</span></div><p className="mt-3 text-2xl font-semibold text-slate-950">{commerce.checkoutsToday ? `${Math.round((commerce.completedOrdersToday / commerce.checkoutsToday) * 100)}%` : '0%'}</p><p className="mt-1 text-xs text-slate-500">Completed orders from today's checkout activity</p></div>
    </section>
