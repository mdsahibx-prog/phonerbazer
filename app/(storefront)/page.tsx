...
function SectionFallback({ className = 'min-h-[180px]' }: { className?: string }) {
  return <div className={`mx-auto max-w-7xl px-3 sm:px-6 lg:px-8 ${className}`} aria-hidden="true" />
}
...
async function FeaturedProductsSection() {
  ...
  const products = featuredProducts.length ? featuredProducts : await getHomepageLatestProducts()
  return (
...
async function DealsSection() {
  const products = await getHomepageDeals()
...
async function BrandsSection() {
  ...
  return (
    <section className="bg-[#f6f7f9] py-5 sm:py-10" aria-labelledby="brands-heading">
      <div className="mx-auto max-w-7xl px-3 sm:px-6 lg:px-8">
...
export default async function HomePage() {
  ...
  return (
    <main className="flex-1 overflow-x-hidden bg-[#f6f7f9]">
      <HeroSection banners={hero.banners} productCount={hero.productCount} brandCount={hero.brandCount} categoryCount={hero.categoryCount} />

      <section className="border-b border-slate-200 bg-[#f6f7f9] py-3 sm:py-4" aria-label="Shopping benefits">
      ...
      <Suspense fallback={<SectionFallback className="min-h-[360px]" />}><CategorySection /></Suspense>
      <Suspense fallback={<SectionFallback className="min-h-[520px]" />}><FeaturedProductsSection /></Suspense>
      <Suspense fallback={<SectionFallback className="min-h-[360px]" />}><DealsSection /></Suspense>
      <Suspense fallback={<SectionFallback className="min-h-[280px]" />}><BrandsSection /></Suspense>

      <section className="bg-[#172033] py-7 text-white sm:py-12">
