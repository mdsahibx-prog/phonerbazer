...
function readableError(code: string | null, description: string | null) {
  if (code === 'otp_expired') {
    return 'This invitation link has expired or was already used. Request a new invitation and open the newest link once.'
  }

  return (
    description?.replace(/\+/g, ' ') ||
    'We could not complete the authentication request. Please request a new invitation.'
  )
}
  ...
  useEffect(() => {
    ...
    async function completeAuthentication() {
      ...
      if (sessionError || !data.session) {
        if (active) {
          setError(readableError(null, sessionError?.message ?? null))
        }
        return
      }
  ...
  return (
    <main className="grid min-h-screen place-items-center bg-slate-100 px-4">
      <section className="w-full max-w-md rounded-2xl border border-slate-200 bg-white p-7 text-center shadow-sm">
        {error ? (
          <>
            <span className="mx-auto flex h-12 w-12 items-center justify-center rounded-xl bg-rose-50 text-rose-700">
            ...
            <a
              href="/admin/login"
              className="mt-6 inline-flex h-10 items-center rounded-lg bg-slate-950 px-4 py-2 text-sm font-semibold text-white"
            >
          ...
          <>
            ...
            <p className="mt-3 text-sm leading-6 text-slate-600">
              Your invitation is being verified with the isolated SahiGatget Auth service.
            </p>
            <Loader2 className="mx-auto mt-6 h-5 w-5 animate-spin text-emerald-700" />
          </>
        )}
      </section>
    </main>
